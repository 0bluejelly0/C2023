...
int main() {
	...
	char name[21];                       // 이름을 저장할 문자열 배열 선언 (최대 20글자까지 입력가능)
   	char birth[21];                      // 생년월일을 저장할 문자열 배열 선언 (최대 20글자까지 입력가능)
	char studentid[21];                  // 학번을 저장할 문자열 배열 선언 (최대 20글자까지 입력가능)
	char department[21];                 // 학과명을 저장할 문자열 배열 선언 (최대 20글자까지 입력가능)
	...
	printf("학과명: %s\n", department);    // 입력받은 학과명을 출력하고 한줄을 띈다
